...
if __name__ == '__main__':
    ...
    for param in model.parameters():                        # froze pre-trained weights
        param.requires_grad = False     
        
    lora_config = LoraConfig(                               # equip with LoRA
    ...
    for param in model.classifier.parameters():             # open the classifier's weights
    ...
    state_dict = torch.load(os.path.join(args.save_dir,"predict_head_weights.pth"),weights_only=True)  # 
    test_model.classifier.load_state_dict( state_dict['PREDICT_HEAD'] )
    test_model = PeftModel.from_pretrained(test_model, os.path.join(args.save_dir,"lora_adapter"))
